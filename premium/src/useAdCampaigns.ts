...
const REMOTE_CONFIG_URL = "/campaigns.json";
  ...
  useEffect(() => {
    ...
    const checkCampaigns = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          remoteCampaigns = (data.active_campaigns || []).filter((c: any) => {
            ...
            // Targeting booleans
            if (c.targeting?.requires_premium === false && isPremium)
              return false;
            if (c.targeting?.requires_premium === true && !isPremium)
              return false;
            ...
            if (c.targeting?.max_version) {
              ...
              const CURRENT_APP_VERSION =
                import.meta.env.VITE_APP_VERSION || "2.0.1";
              if (
                !isVersionLessThanOrEqual(
                  CURRENT_APP_VERSION,
                  c.targeting.max_version,
                )
              ) {
            ...
            const dismissedIds = JSON.parse(
              localStorage.getItem("natively_dismissed_campaigns") || "[]",
            );
        ...
        console.warn(
          "[AdCampaigns] Remote fetch failed, falling back to local.",
        );
      }
      ...
      let history: string[] = JSON.parse(
        localStorage.getItem("natively_ads_shown_history") || "[]",
      );
      let availableCandidates = candidates.filter(
        (c) => !history.includes(c.id),
      );
      ...
      const topCandidates = availableCandidates.filter(
      ...
      if (!isMounted) return;
      setPendingAd({
        ad: selectedCandidate.ad,
        id: selectedCandidate.id,
        readyAt: nowTime + delayToUse,
      });
    };
